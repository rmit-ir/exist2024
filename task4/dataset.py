...
DATA_PATH= "path_to_your_text dataset folder" #"../data"
DATA_PATH_IMAGE = "path_to_your_image_folder"
...
class MyDataset(Dataset):
    def __init__(self, mode, limit=None):
        ...
        for id in self.data.keys():
            self.data[id]["image_path"] = os.path.join(DATA_PATH_IMAGE,str(id)+".jpeg")#os.path.join("../memes",str(id)+".jpeg")
    
    def load_data(self, mode, limit):
        cnt = 0
        data_set=dict()
        if mode in ["train"]:
            f1= open(os.path.join(DATA_PATH, mode+".json"),'r',encoding='utf-8')
            datas = json.load(f1)
            for data in datas:
                ...
                if os.path.isfile(os.path.join(DATA_PATH_IMAGE,str(image)+".jpeg")):
        ...
        if mode in ["test","valid"]:
            ...
            for data in datas:
                ...
                if os.path.isfile(os.path.join(DATA_PATH_IMAGE,str(image)+".jpeg")): #path_to_your_image_folder
                    #os.path.isfile(os.path.join("../memes",str(image)+".jpeg"))
                    data_set[int(image)]={"text":sentence, 'label': label}
                    cnt += 1
        return data_set
